...
function XacNhan(nFunction)
	local selTab = {
				"Ta ®ång ý/"..nFunction.."",
				"KÕt thóc ®èi tho¹i/nothing",
				}
	Say("ChiÕn tr­êng n¬i ®Çy nguy hiÓm b¹n cã muèn ra!",getn(selTab),selTab);
end
...
function main()
	...
	local selTab = {
				" ®· tiÕn vµo hËu doanh/#XacNhan('enter_back_camp')",						
				"Rêi khái chiÕn tr­êng/ask_reason",
				"KÕt thóc ®èi tho¹i/nothing",
				}
	local nCamp = BT_GetCamp()
	if nCamp == SONG_ID and GetMissionV(MV_MID_TRAP_SONG) == 1 then
		tinsert(selTab,2," tiÕn vµo bªn trong chiÕn tr­êng/#XacNhan('enter_mid_camp')");
	...
	if nCamp == SONG_ID and GetMissionV(MV_FRONT_TRAP_SONG) == 1 then
		tinsert(selTab,3," ®· tiÕn vµo tiÒn doanh/#XacNhan('enter_front_camp')");
