...
function TY_EnterCondCheck(szNpcName, nMSType, nDifType)
	...
	if tResInfo and getn(tResInfo) > 0 then
		if eDifType.NORMAL == nDifType then
			...
			Talk(1, "", format(szNpcName.."%s cÇn tæ ®éi ®i vµo. Thµnh viªn tæ ®éi ph¶i <color=yellow> ®¼ng cÊp >= cÊp 90, sè l­u ph¸i >=1 vµ ®· gia nhËp l­u ph¸i <color> míi vµo ®­îc", tMSName[nMSType]));
		end
