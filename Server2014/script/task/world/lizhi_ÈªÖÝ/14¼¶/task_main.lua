...
-- ½£ÏÀÇéÔµonlineII ÈªÖÞ 14 ¼¶ÈÎÎñÖ÷Ìå½Å±¾
-- Edited by lizhi
-- 2005-8-4 10:38
-- ======================================================
...
function task_001_00()
    local szMsg = {
        "ThËt ®¸ng tiÕc! chuyÕn hµng b»ng ngäc mÊt hÕt. Ta chØ ph¸t hiÖn bøc th­ trong phßng cña L­u qu¶n gia, ch¾c trong lóc gÊp g¸p «ng ta ®Ó quªn. Ch÷ trong th­ ®· mê do dÝnh n­íc, ng­¬i thö mang ®i ®iÒu tra xem.",
	...
	AddItem(2, 0, 38, 1);   --Ä£ºýµÄ×ÖÌõ
	
	SetTask(TASK_LEV_14_ID, 1);
	SetTask(TASK_LEV_12_ID, GetTask(TASK_LEV_12_ID) + 1);   --Ïû³ýÈÎÎñÃæ°å
end;
...
function task_003_00()
	...
	GivePlayerExp(SkeyQZWangyewei,"jiayouneizei1")		--¼ÒÓÐÄÚÔô1
    SetTask(TASK_LEV_14_ID, 3);
    
    DelItem(2,2,65,1);
    DelItem(2,2,30,1);
    DelItem(2,0,39,1);
    DelItem(2, 0, 38, 1);   --Ä£ºýµÄ×ÖÌõ
...
function task_004_00()
	...
	GivePlayerExp(SkeyQZWangyewei,"jiayouneizei2")		--¼ÒÓÐÄÚÔô2
    SetTask(TASK_LEV_14_ID, 4);
    SetTask(TASK_LEV_18_ID, 100);   --ÈªÖÝÖÓ·ÅÈÎÎñ¿ªÆô
end;

function task_004_01()
    
    local szTalk = {
        "TÊt c¶ lµ do Chung Phãng chØ thÞ, ng­¬i t×m h¾n hái xem, xin tha m¹ng!"
    };
    TalkEx("", szTalk);
end;