...
function main()
	if GetTrigger(3000) == 0 then
		...
		Say("Ho¹t ®éng Gi¸ng Sinh vÉn ch­a më. Thêi gian chÝnh thøc lµ: <color=yellow>tõ 23/11 ®Õn 28/12<color>.",0)
		return 0
	end
	SelTab = {
...
function get_present()
	...
	if(GetTask(TASK_GOT_PRESENT) == 0) then --Ö®Ç°Ã»ÓÐÁì¹ý
...
function give_present()
	...
	if(nPresentRemain > 0) then
		...
		elseif nTemp > 20 and nTemp <= 30 then
			nRetCode = AddItem(2,1,148,5,1)	--Ð¡¿é½ðÏ¬5¸ö
		elseif nTemp > 30 and nTemp <= 40 then
			nRetCode = AddItem(2,1,209,1,1)	--Ê¥µ®ÀñºÐ1¸ö
		...
		elseif nTemp > 60 and nTemp <= 70 then
			nRetCode = AddItem(2,1,211,1,1)	--"ºØ¿¨¶þ 1¸ö"
		...
		elseif nTemp > 90 and nTemp <= 100 then --5¸öËæ»úÑæ»ð
			for i=1,5 do			
		...
		if nRetCode == 0 then	--Ã»ÓÐ³É¹¦Ìí¼Ó
		...
		Say("Xin lçi, quµ ®· bÞ lÊy hÕt, lÇn sau h·y quay l¹i!",0)
	end
	if nEmpty == 1 then
		AddGlobalNews("Quµ c©y th«ng Gi¸ng Sinh ë B¾c TuyÒn Ch©u ®· bÞ lÊy hÕt, ng­êi ch¬i nµo ch­a lÊy ®­îc h·y ®Õn lÇn sau.")
		Msg2SubWorld("Quµ c©y th«ng Gi¸ng Sinh ë B¾c TuyÒn Ch©u ®· bÞ lÊy hÕt, ng­êi ch¬i nµo ch­a lÊy ®­îc h·y ®Õn lÇn sau.")
	end
end
